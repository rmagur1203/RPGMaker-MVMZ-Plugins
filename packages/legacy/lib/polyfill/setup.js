...
function variablify(obj, name) {
  ...
  if (typeof obj === "object") {
    let escapedName = name.replace(/[^a-zA-Z0-9_]/g, "_");
    result += `const ${escapedName} = ${JSON.stringify(obj, null, 4)};\n`;
    // for (const key in obj) {
    //     result += variablify(obj[key], escapedName + "_" + key);
    // }
  }
  return result;
}
...
writeFileSync(
  join(root, "js", "fs.js"),
  variablify(content.readdir, "readdir"),
  "utf8"
);
